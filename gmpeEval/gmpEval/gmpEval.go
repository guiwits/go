...
// Go likes names to start with a capital letter for exported functions
func GmpEval(oLat float64, oLon float64, oData []string,
	pData []string, tolerance float64, aTime float64, threshold float64, gmType string) (int, error) {
	...
	for i := 0; i < len(oData); i++ {
		lat, _ := strconv.ParseFloat(strings.Split(oData[i], " ")[1], 64)
		lon, _ := strconv.ParseFloat(strings.Split(oData[i], " ")[0], 64)

		//// Again generalize to gm and [4] for mmi, [2] for pga, [3] for pgv
		mmi, _ := strconv.ParseFloat(strings.Split(oData[i], " ")[4], 64)
